...
```{r}
sarima(PM2.5, p=19, d=0, q=0)
```

The diagnostic plots for AR(19) show that the residuals have no significant autocorrelations, follow roughly normal, and all Ljung-Box statistics are larger than the threshold p-value. AR(19) seems to track the series well with the proper residuals.

Seasonal ARIMA

The decomposition of the time series suggests that there is a daily seaonality in a year. 
...
Alternatively, ARMA with yearly dummy:

So, we include year dummy for 2017, 2018, .., 2020 in AR model to account for any difference in the intercept in AR model in different years. This is a way to do with sarima(..., P=1, D=0, Q=0, s=365.25) because s only takes less than 350

```{r}
year_dummies <- dummy_cols(airquality$Year)
names(year_dummies) <- c("year", "2016", "2017", "2018", "2019", "2020")

AICs <- c()
for (p in 1:30) {
  AICs <- c(AICs, arima(PM2.5, order=c(p, 0, 0), xreg=year_dummies[,c("2017", "2018", "2019", "2020")])$aic)
  cat("AR(", p, "): AIC=",AICs[p], "\n")
}
...
With yearly dummy variables, AR(19) has the smallest AIC of 14077.08.


```{r}
arima(PM2.5, order=c(19, 0, 0), xreg=year_dummies[,c("2017", "2018", "2019", "2020")])
```


we can manually examine yearly lag airquailty obtained by taking a differene with 365th lagged term.
...
The AIC tells that the seasnality removed series follows AR(p) of order only p = 5 instead of 19,


```{r}
arima(yearly_lagged_PM2.5, order=c(5, 0, 0))
```
This model has a AIC of 12240.69 and the best model so far.
...
fit$AIC 
```
Creating BATS model, however in that model, the AIC is a lot bigger than the AR(19) with seasonality taken into account. BATS model is more generalzed version of ARIMA just like ARMA is a generalizatino of AR and ARIMA is a generalization of ARMA. 
